...
def main():
    parser = argparse.ArgumentParser(description="campus.flow administration")
    sub = parser.add_subparsers(dest="command", required=True)
    add = sub.add_parser("create-officer")
        ...
        if args.command == "backup":
            ...
            if args.command == "create-officer":
                ...
                user = connection.execute(
                    "SELECT id FROM users WHERE email=?", (args.email.lower(),)
                ).fetchone()
                if not user:
                ...
                connection.execute(
                    "INSERT INTO audit(action,entity_id) VALUES('password.admin_reset',?)",
                    (user["id"],),
                )
