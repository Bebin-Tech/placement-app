...
class PlacementApiTest(unittest.TestCase):
    ...
    def test_csrf_password_logout_and_health(self):
        ...
        local=backend.app.test_client().post('/api/auth/login',json={'email':'student@test.com','password':'Changed-password-456'},headers={'Origin':'http://localhost:5174'});self.assertEqual(local.status_code,200)
        loopback=backend.app.test_client().post('/api/auth/login',json={'email':'student@test.com','password':'Changed-password-456'},headers={'Origin':'http://127.0.0.1:5199'});self.assertEqual(loopback.status_code,200)
        evil=backend.app.test_client().post('/api/auth/login',json={'email':'student@test.com','password':'Changed-password-456'},headers={'Origin':'https://evil.example'});self.assertEqual(evil.status_code,403)
        deceptive=backend.app.test_client().post('/api/auth/login',json={'email':'student@test.com','password':'Changed-password-456'},headers={'Origin':'http://127.0.0.1.evil.example:5173'});self.assertEqual(deceptive.status_code,403)
